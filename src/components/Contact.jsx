...
function Contact({contactModalState, setContactModalState}){
    ...
    return(
        <div>
            {/* {copied && 
            <>
            <p className="fixed bottom-0 right-0 z-50">asdasdsaadsasda</p>
            </>
            } */}
            <ul className="w-full h-60 flex justify-evenly items-center flex-wrap flex-col">
                {contactItems.map((item) => (
                    <li key={item.id} onClick={() => setContactState(item.id)} className={contactState == item.id ? `${item.style}  ${item.styleActive} shadow-lg` : `shadow-lg ${item.style}`}>
                        <div className={contactState == item.id ? "w-full h-full flex justify-between items-center pl-4 pr-4 aparece" : "desaparece"}>
                            <a className={`text-${item.colorText} underline`}>{item.value}</a>
                        ...
                        <a className={contactState == item.id ? "opacity-0" : `opacity-1 text-${item.colorText}`}><FontAwesomeIcon className="text-2xl" icon={item.icon} /></a>
                    </li>
                ))}
                    ...
                    <div className={contactState == 1 ? "w-full h-full flex justify-between items-center pl-4 pr-4 aparece" : "desaparece"}>
                        ...
                        <div className="flex h-full">
                            <CopyToClipboard text={"+54 115964-0417"} onCopy={() => setCopied(true)}>
                                <svg onClick={() => setCopied(true)} fill="white" xmlns="http://www.w3.org/2000/svg" className="w-5 mr-2" viewBox="0 0 512 512"><path d="M224 0c-35.3 0-64 28.7-64 64V288c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V64c0-35.3-28.7-64-64-64H224zM64 160c-35.3 0-64 28.7-64 64V448c0 35.3 28.7 64 64 64H288c35.3 0 64-28.7 64-64V384H288v64H64V224h64V160H64z"/></svg>
                            </CopyToClipboard>
                ...
                <div className={contactState == 3 ? "w-full h-full flex justify-between items-center pl-4 pr-4 aparece" : "desaparece"}>
                    ...
                    <div className="flex h-full">
                        <CopyToClipboard text="https://www.linkedin.com/in/matias-seitour-1a0118240/" onCopy={() => setCopied(true)}>
                            <svg onClick={() => setCopied(true)} fill="blue" xmlns="http://www.w3.org/2000/svg" className="w-5 mr-2" viewBox="0 0 512 512"><path d="M224 0c-35.3 0-64 28.7-64 64V288c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V64c0-35.3-28.7-64-64-64H224zM64 160c-35.3 0-64 28.7-64 64V448c0 35.3 28.7 64 64 64H288c35.3 0 64-28.7 64-64V384H288v64H64V224h64V160H64z"/></svg>
                        </CopyToClipboard>
